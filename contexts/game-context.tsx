...
export const FREE_PROFILE_ICONS = PROFILE_ICONS.map((i) =
 ({ ...i, free: true as const }))
...
export function findShopIconByImage(image?: string | null): ProfileIcon | undefined {
  ...
  return SHOP_PROFILE_ICONS.find((i) =
 i.image === image)
}
...
interface GameContextType {
 ...
 boolean
  ...
  addStaminaBottles: (amount: number) =
 void
 ...
 Card | null
  addGift: (gift: Omit<GiftBox, "id" | "claimed"
) =
 void
  hasUnclaimedGifts: boolean
  playerId: string
  playerProfile: PlayerProfile
  updatePlayerProfile: (updates: Partial<PlayerProfile
) =
 void
  friends: Friend[]
 ...
 boolean
  sendHeartToAll: () =
 number
  likeFriendShowcase: (friendId: string) =
 void
  spendFriendPoints: (amount: number) =
 boolean
  searchPlayerById: (id: string) =
 Friend | null
  getGhostPlayers: (count: number) =
 Friend[]
  canSendHeartTo: (friendId: string) =
 boolean
 ...
 Promise<{ success: boolean; error?: string }
  ...
  loginWithCode: (code: string, password: string) =
 Promise<{ success: boolean; error?: string }

  registerWithCode: (password: string) =
 Promise<{ success: boolean; error?: string; code?: string }

  linkEmailToCode: (email: string) =
 Promise<{ success: boolean; error?: string }
 ...
 void
  ...
  setGlobalPlaymat: (playmatId: string | null) =
 void
  getPlaymatForDeck: (deck: Deck) =
 Playmat | null
  unlockPlaymat: (playmatId: string) =
 boolean
  allSleeves: Sleeve[]
  ownedSleeves: Sleeve[]
  globalSleeveId: string | null
  setGlobalSleeve: (sleeveId: string | null) =
 void
  unlockSleeve: (sleeveId: string) =
 boolean
  // Icones de perfil (avatares)
  shopProfileIcons: ProfileIcon[]
  ownedIconIds: string[]
  ownsProfileIcon: (iconId: string) =
 boolean
  unlockProfileIcon: (iconId: string) =
 boolean
  equipProfileIcon: (iconId: string) =
 boolean
...
([])
  const [globalPlaymatId, setGlobalPlaymatId] = useState<string | null
(null)
  const [ownedSleeves, setOwnedSleeves] = useState<Sleeve[]
([])
  const [globalSleeveId, setGlobalSleeveId] = useState<string | null
(null)
...
([])
  ...
  const getMaxStamina = (level: number) =
 19 + level
...
= STAMINA_REGEN_SECS) {
  ...
  const refillStamina = () =
 {
  ...
  const staminaNextTickSeconds = stamina 
= getMaxStamina(playerProfile.level)
    ? 0
            ...
            if (result.success && result.progress) {
              ...
              if (p && typeof p === "object") {
                ...
                if (Array.isArray(p.friends)) {
                  const hasGuest = p.friends.some((f: Friend) =
 f.id === "GUEST-001")
                ...
                if (Array.isArray(p.ownedPlaymatIds)) {
                  setOwnedPlaymats(ALL_PLAYMATS.filter((pm) =
 p.ownedPlaymatIds.includes(pm.id)))
                ...
                if (Array.isArray(p.ownedSleeveIds)) {
                  setOwnedSleeves(ALL_SLEEVES.filter((s) =
 p.ownedSleeveIds.includes(s.id)))
                ...
                if (Array.isArray(p.ownedIconIds)) {
                  const ids = p.ownedIconIds.filter((id: unknown) =
 typeof id === "string")
                  setOwnedIconIds(ids)
 ...
 {
            const saved = parsed.find((p) =
 p.id === gift.id)
            return saved ? { ...gift, claimed: saved.claimed } : gift
          })
          const newGifts = parsed.filter((p) =
 !INITIAL_GIFT_BOXES.find((g) =
 g.id === p.id))
 ...
 f.id === "GUEST-001")
      ...
      if (savedOwnedPlaymats) {
        try {
          ...
          setOwnedPlaymats(ALL_PLAYMATS.filter((p) =
 playmatIds.includes(p.id)))
        } catch { }
      }
 ...
 sleeveIds.includes(s.id)))
      ...
      if (savedOwnedIcons) {
        try {
          ...
          if (Array.isArray(iconIds)) setOwnedIconIds(iconIds.filter((id: unknown) =
 typeof id === "string"))
        } catch { }
      }
    ...
    setFragments((prev) =
 {
  ...
  const getFragmentCount = useCallback((id: FragmentId) =
 fragments[id] ?? 0, [fragments])
    ...
    setChests((prev) =
 {
  ...
  const getChestCount = useCallback((id: ChestId) =
 chests[id] ?? 0, [chests])
 ...
 Math.max(0, prev - 1))
    setStamina((prev) =
 Math.min(max, prev + STAMINA_BOTTLE_REFILL_AMOUNT))
    return true
    ...
    setXpBooks((prev) =
 {
  ...
  const getXPBookCount = useCallback((id: XPBookId) =
 xpBooks[id] ?? 0, [xpBooks])
  ...
  // useEffect(() =
 {
  ...
  //     setAccountAuth((prev) =
 ({ ...prev, lastSaved: new Date().toISOString() }))
  //   }
  // }, [accountAuth])
  ...
  useEffect(() =
 {
  ...
  useEffect(() =
 {
    localStorage.setItem("gearperks_owned_playmats", JSON.stringify(ownedPlaymats.map((p) =
 p.id)))
  }, [ownedPlaymats])
  ...
  useEffect(() =
 {
  ...
  useEffect(() =
 {
    localStorage.setItem("gearperks_owned_sleeves", JSON.stringify(ownedSleeves.map((s) =
 s.id)))
  }, [ownedSleeves])
...
= 0) {
 ...
 (g.id === giftId ? { ...g, claimed: true } : g)))
    ...
    if (gift.playmatId) {
      const playmat = ALL_PLAYMATS.find((p) =
 p.id === gift.playmatId)
      if (playmat && !ownedPlaymats.some((p) =
 p.id === playmat.id)) {
        setOwnedPlaymats((prev) =
 [...prev, playmat])
      }
      return null
    }
 ...
 [...prev, newGift])
  ...
  const hasUnclaimedGifts = giftBoxes.some((g) =
 !g.claimed)
...
= maxPoints
 ...
 prev + 5)
    ...
    if (!friends.find((f) =
 f.id === friendId)?.isGuest || friendId === "GUEST-001") {
      addGift({
 ...
 ({
    ...
    playerProfile,
    playerId,
    ownedPlaymatIds: ownedPlaymats.map((p) =
 p.id),
    globalPlaymatId,
    ownedSleeveIds: ownedSleeves.map((s) =
 s.id),
    globalSleeveId,
    ownedIconIds,
    redeemedCodes,
 ...
 typeof id === "string")
  ...
  const accountApi = async (payload: Record<string, unknown
): Promise<any
 =
 {
    try {
  ...
  const saveProgressManually = async () =
 {
    ...
    const payload = { ...rest, ownedPlaymatIds: mats.map((p) =
 p.id), ownedSleeveIds: sleeves.map((s) =
 s.id) }
 ...
 {
    const sleeve = ALL_SLEEVES.find((s) =
 s.id === sleeveId)
    if (!sleeve) return false
    if (ownedSleeves.some((s) =
 s.id === sleeveId)) return false
    const updated = [...ownedSleeves, sleeve]
    setOwnedSleeves(updated)
    localStorage.setItem("gearperks_owned_sleeves", JSON.stringify(updated.map((s) =
 s.id)))
    return true
  }

  // ── ICONES DE PERFIL (avatares) ──────────────────────────────────────────
  const ownsProfileIcon = (iconId: string): boolean =
 ownedIconIds.includes(iconId)

  /** Libera um icone da loja na conta do jogador (usado nas compras). */
  const unlockProfileIcon = (iconId: string): boolean =
 {
    const icon = SHOP_PROFILE_ICONS.find((i) =
 i.id === iconId)
 ...
 {
    const free = PROFILE_ICONS.find((i) =
 i.id === iconId)
    if (free) {
      setPlayerProfile((prev) =
 ({ ...prev, avatarUrl: free.image }))
      return true
    }
    const shopIcon = SHOP_PROFILE_ICONS.find((i) =
 i.id === iconId)
    if (!shopIcon || !ownedIconIds.includes(iconId)) return false
    setPlayerProfile((prev) =
 ({ ...prev, avatarUrl: shopIcon.image }))
    return true
  }
 ...
 ({ id: i.id, name: i.name, image: i.image })),
    ...SHOP_PROFILE_ICONS.filter((i) =
 ownedIconIds.includes(i.id)).map((i) =
 ({
      id: i.id,
      name: i.name,
      image: i.image,
 ...
 {
    if (globalSleeveId) {
      const sleeve = ownedSleeves.find((s) =
 s.id === globalSleeveId)
 ...
 p.id === playmatId)
    ...
    if (ownedPlaymats.some((p) =
 p.id === playmatId)) return false
    const updated = [...ownedPlaymats, playmat]
    setOwnedPlaymats(updated)
    localStorage.setItem("gearperks_owned_playmats", JSON.stringify(updated.map((p) =
 p.id)))
    return true
  }
  ...
  const getPlaymatForDeck = (deck: Deck): Playmat | null =
 {
    ...
    if (deck.useGlobalPlaymat !== false && globalPlaymatId) {
      return ownedPlaymats.find((p) =
 p.id === globalPlaymatId) || null
    }
    // If deck has specific playmat
    if (deck.playmatId) {
      return ownedPlaymats.find((p) =
 p.id === deck.playmatId) || null
    }
    // Fallback to global
    if (globalPlaymatId) {
      return ownedPlaymats.find((p) =
      ...
      ALL_CARDS.forEach((card) =
 {
    ...
    if (normalizedCode === "PLAYMAT") {
      const newOwnedPlaymats = ALL_PLAYMATS.filter(
        (p) =
 !ownedPlaymats.some((op) =
 op.id === p.id)
      )
      const updatedPlaymats = [...ownedPlaymats, ...newOwnedPlaymats]
      setOwnedPlaymats(updatedPlaymats)
      localStorage.setItem("gearperks_owned_playmats", JSON.stringify(updatedPlaymats.map((p) =
 p.id)))
    ...
    if (normalizedCode === "SLEEVES") {
      const newOwnedSleeves = ALL_SLEEVES.filter(
        (s) =
 !ownedSleeves.some((os) =
 os.id === s.id)
      )
      const updatedSleeves = [...ownedSleeves, ...newOwnedSleeves]
      setOwnedSleeves(updatedSleeves)
      localStorage.setItem("gearperks_owned_sleeves", JSON.stringify(updatedSleeves.map((s) =
 s.id)))
