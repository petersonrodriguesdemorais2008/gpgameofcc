...
export default function GachaScreen({ onBack }: GachaScreenProps) {
  ...
  const drawParticles = useCallback(() => {
    ...
    const animate = () => {
      ...
      // Opening burst — a single restrained wave of upward embers. The "big moment"
      // visuals now live in the CSS layer (energy ring, light sweeps, shockwave
      // rings) — the canvas only adds a light dusting of embers on top, not a
      // second competing burst. No spawnRing here: a ring of particles expanding
      // from a point is the single most "firework chrysanthemum"-looking pattern
      // there is, so it's avoided entirely.
      if(packPhase==="opening") {
        if(t===1) spawnBurst(22,cx,cy,14)
        if(t===8) spawnBurst(10,cx,cy,8)
      }

      // Revealing — a small twinkle only on rare cards; plain R cards get no
      // canvas burst at all (the flip itself is the whole event for those).
      if(packPhase==="revealing" && cardRevealIndex>=0) {
        if(t===2 && revealingCardRarity) {
          const n = revealingCardRarity==="LR" ? 14 : revealingCardRarity==="UR" ? 9 : 6
          spawnBurst(n, cx + (cardRevealIndex-1.5)*80, cy, revealingCardRarity==="LR"?10:7)
        }
      }
    ...
    animate()
  }, [packPhase, rarityTier, cardRevealIndex, revealingCardRarity])
